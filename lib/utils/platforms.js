...
function detectPlatform(platform) {
    ...
    if (_.contains(name, 'linux')
        ...
        else if (_.contains(name, 'linux') || hasSuffix(name, '.tgz') || hasSuffix(name, '.tar.gz') || hasSuffix(name, '.appimage') || hasSuffix(name, '.snap')) {
    ...
    // FORCE suffix in our case
    // https://github.com/GitbookIO/nuts/issues/121
    if (prefix === platforms.WINDOWS) {
        suffix = '32';
    } else if (prefix === platforms.OSX) {
        suffix = '64';
    }
